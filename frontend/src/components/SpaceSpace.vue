<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            공간 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            공간
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            ...
            <Boolean label="정비여부" v-model="value.isMaintenance" :editMode="editMode" :inputUI="''"/>
            <Boolean label="예약여부" v-model="value.isReserve" :editMode="editMode" :inputUI="''"/>
            <String label="매니저정부" v-model="value.managerId" :editMode="editMode" :inputUI="''"/>
        </v-card-text>
